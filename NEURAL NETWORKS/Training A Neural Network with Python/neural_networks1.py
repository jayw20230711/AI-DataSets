...
class NeuralNetwork:
    ...
    def train(self, input_vector, target_vector):
        ...
        input_vector = input_vector.reshape(input_vector.size, 1)         # now shape is  ex: (3, 1)
        target_vector = np.array(target_vector).reshape(target_vector.size, 1)   # shape is  Ex: (2, 1)
        ...
        self.weights_in_hidden += self.learning_rate * (tmp @ input_vector.T)      # reverse previous T of input vector
    ...
    def evaluate(self, data, labels):
        ...
        A result is considered to be correct, if the index of the maximal value corresponds
        to the index with the "1" in the one-hot representation,
        e.g
        res = [0.1, o.132, 0.875]
        labels[i] = [0, 0, 1]
        """
        ...
        for i in range(len(data)):
            res = self.run(data[i])          # call run function
            res_max = res.argmax()
